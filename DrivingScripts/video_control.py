...
def game_loop(args):
    ...
    try:
        ...
        temp = world.camera_manager.img_array
        shape = (temp[0].shape[1], temp[0].shape[0])
        fps = len(temp) / (time.time() - world.camera_manager.start)
        out = cv2.VideoWriter('recording.avi', cv2.VideoWriter_fourcc(*'DIVX'), fps, shape)
        for idx in range(len(temp)):
            print(idx)
            img = cv2.cvtColor(temp[idx], cv2.COLOR_BGR2RGB)
            out.write(img)
        out.release()
